...
export default function ProveedorForm({ defaultValues, onSubmit, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1">Persona de contacto</label>
        <input type="text" {...register('contacto')} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm" />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-text-primary mb-1">Teléfono</label>
          <input type="text" {...register('telefono')} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm" />
        </div>
        <div>
          <label className="block text-sm font-medium text-text-primary mb-1">Email</label>
          <input type="email" {...register('email')} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm" />
          {errors.email && <p className="text-danger text-xs mt-1">{errors.email.message}</p>}
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1">Dirección</label>
        <textarea {...register('direccion')} rows={2} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm resize-none" />
      </div>
