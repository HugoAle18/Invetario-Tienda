...
function ChartCard({ title, children, className = '' }) {
  return (
    <div className={`bg-bg-card border border-bg-border rounded-xl p-5 ${className}`}>
      <h3 className="font-display font-semibold text-text-primary mb-4">{title}</h3>
      {children}
    </div>
  )
}
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-80 bg-bg-card border border-bg-border rounded-xl animate-pulse" />
        ))}
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center py-20 text-text-secondary">
        ...
        <button onClick={fetchAll} className="flex items-center gap-2 px-4 py-2 bg-brand hover:bg-brand-hover text-white rounded-lg text-sm">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-display font-bold text-text-primary">Reportes</h2>
        </div>
      </div>

      {/* Date range for movements */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-bg-card border border-bg-border rounded-xl p-4">
        <span className="text-sm text-text-secondary font-medium">Rango de movimientos:</span>
        <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)}
          className="px-3 py-1.5 bg-bg-primary border border-bg-border rounded-lg text-text-primary text-sm focus:border-brand focus:ring-1 focus:ring-brand outline-none" />
        <span className="text-text-muted text-sm">a</span>
        <input type="date" value={hasta} onChange={(e) => setHasta(e.target.value)}
          className="px-3 py-1.5 bg-bg-primary border border-bg-border rounded-lg text-text-primary text-sm focus:border-brand focus:ring-1 focus:ring-brand outline-none" />
        <button onClick={fetchAll} className="px-4 py-1.5 bg-brand hover:bg-brand-hover text-white text-sm rounded-lg transition-colors">
          Actualizar
        </button>
      </div>
                ...
                <div key={item.nombre} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-text-primary truncate">{item.nombre}</span>
                      <span className="text-xs text-text-secondary shrink-0 ml-2">{item.total_movimientos} mov.</span>
                    </div>
      ...
      {vals.length > 0 && (
        <div className="bg-bg-card border border-bg-border rounded-xl p-5">
          <h3 className="font-display font-semibold text-text-primary mb-4">Desglose por Categoría</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {vals.map((c) => (
                  <tr key={c.nombre} className="border-b border-bg-border hover:bg-bg-hover transition-colors">
                    <td className="py-3 px-3 text-text-primary font-medium">{c.nombre}</td>
                    <td className="py-3 px-3 text-right text-text-secondary">{c.productos}</td>
                    <td className="py-3 px-3 text-right text-text-secondary">{c.total_stock}</td>
                    <td className="py-3 px-3 text-right text-text-primary font-mono">{formatCurrency(c.valor_compra)}</td>
                    <td className="py-3 px-3 text-right text-text-primary font-mono">{formatCurrency(c.valor_venta)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
