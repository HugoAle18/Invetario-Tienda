    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1">
          Contraseña {isEditing && <span className="text-text-muted font-normal">(dejar vacío para mantener)</span>}
        </label>
        <input type="password" {...register('password')} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm" />
        {errors.password && <p className="text-danger text-xs mt-1">{errors.password.message}</p>}
      </div>
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1">Rol</label>
        <select {...register('rol')} className="w-full px-3 py-2 bg-bg-primary border border-bg-border rounded-lg text-text-primary focus:border-brand focus:ring-1 focus:ring-brand outline-none text-sm">
