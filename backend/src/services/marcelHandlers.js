...
export async function handleConsultaProducto(mensaje, usuario) {
  ...
  if (productos.length > 1) {
    let respuesta = 'Encontre varios productos similares:\n\n'
    productos.forEach((p) => {
      respuesta += `\u2022 ${p.nombre} (${p.codigo})\n`
    })
  ...
  const emoji = stock === 0 ? '' : stock <= minimo ? '' : ''

  let respuesta = `${emoji} ${p.nombre} (${p.codigo})\n\n`
  respuesta += `Estado: ${estado}\n`
  respuesta += `Stock actual: ${stock} unidades\n`
  respuesta += `Stock minimo: ${minimo} unidades\n`
  respuesta += `Categoria: ${p.categorias?.nombre || 'Sin categoria'}\n`
  respuesta += `Proveedor: ${p.proveedores?.nombre || 'Sin proveedor'}\n`
...
export async function handleMovimientosRecientes(mensaje, usuario) {
  ...
  data.forEach((m) => {
    ...
    respuesta += `\u2022 [${hora}] ${emoji} ${m.productos?.nombre || 'Producto'}: ${m.cantidad} unidades (${m.motivo || 'Sin motivo'})\n`
  })
