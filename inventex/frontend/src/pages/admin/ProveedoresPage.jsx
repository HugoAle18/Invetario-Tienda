    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-display font-bold text-white">Proveedores</h2>
          {!loading && <span className="text-sm text-white/50 bg-white/[0.06] px-2 py-0.5 rounded-full">{proveedores.length}</span>}
        </div>
      ...
      {loading && !error && (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-24 bg-white/[0.04] border border-white/[0.06] rounded-xl animate-pulse" />
          ))}
        </div>
      )}
      ...
      {!loading && !error && proveedores.length > 0 && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {proveedores.map((prov) => (
            <div key={prov.id} className="glass-card group">
              <div className="flex items-start justify-between mb-2">
                <h4 className="text-white font-medium truncate">{prov.nombre}</h4>
                <div className="flex gap-1 shrink-0 ml-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openEdit(prov)} className="p-1.5 rounded-lg text-white/60 hover:bg-white/[0.05] hover:text-brand transition-colors" title="Editar">
                    <Pencil size={15} />
                  </button>
                  <button onClick={() => handleDelete(prov)} className="p-1.5 rounded-lg text-white/60 hover:bg-white/[0.05] hover:text-danger transition-colors" title="Eliminar">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
              <div className="space-y-1 text-sm text-white/60">
                {prov.contacto && <p className="truncate"><span className="text-white/40">Contacto:</span> {prov.contacto}</p>}
              ...
              {!prov.contacto && !prov.telefono && !prov.email && !prov.direccion && (
                <p className="text-xs text-white/40 italic mt-1">Sin datos de contacto</p>
              )}
            </div>
          ))}
        </div>
